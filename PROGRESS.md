...
## Accessibility-first audit + contrast-as-error (alpha.7, in working tree) — July 4, 2026

Driver: a deep-research pass (105-agent harness) stress-tested the "senior designer" thesis against the mid-2026 market. Verdict: the pain is real but the *sharpest, best-evidenced, budget-backed* slice is **accessibility/WCAG under EU Accessibility Act enforcement** (in force June 2025; live lawsuits; fines to €100k). AI agents ship inaccessible UI by default (WebAIM Million 2026: 95.9% of pages fail WCAG). Competitors already crowd the deterministic-audit lane (Google Labs `design.md`, `accessibility-agents`, `designlang`) — differentiation lives in the *computed, cross-category* checks eslint-plugin-jsx-a11y can't do (contrast ratios, type/spacing/motion drift) + cross-session memory. Decision: keep the "senior designer" identity but make it *opinionated* — lead with an accessibility-first priority order, and treat WCAG AA contrast fails as build-blocking errors (honesty: a contrast fail is an EAA compliance failure).

- [x] **Contrast AA fails → error.** `src/audit/aggregator.ts::buildColorResult` — the WCAG-AA contrast finding severity flips `warn` → `error`. On `examples/broken-project` this moves totals from `error 6 · warn 13` to `error 9 · warn 10` (exit stays 2).
- [x] **Dashboard reordered into priority bands.** `src/audit/dashboard.ts` — three bands: **Fix before you ship** (accessibility + color/contrast = the WCAG surface), **Then clean up** (everything else, severity-then-count sorted), **Clean**. Computed headline "N of M errors are accessibility failures" (fixture: "8 of 9"). `↳ WCAG AA / EU Accessibility Act` compliance line shown only when the WCAG band has findings. Summary line gained a `· exit 2 — fails CI` hint.
- [x] **Markdown report mirrors the bands** + the EAA blockquote (`src/audit/markdown-report.ts`); category headings demoted `##`→`###` under `##` band headers.
- [x] **Tests updated + green.** `test-audit-color-roles` genuine-fail assertions warn→error (+ skip-tests hardened to `error+warn`); `test-audit-dashboard` +4 new assertions (band header, EAA line, accessibility-above-non-WCAG ordering, CI hint). All audit suites pass; web `tsc --noEmit` clean. (The one red suite, `test:e2e`, is the pre-existing gating-store-isolation flake — zero overlap with these files.)
- [x] **Public numbers synced for honesty.** `GetStartedPreview.tsx` on-site demo (now accessibility-first, banded, `24 · 9 · 10 · 5`, Color as ✗), `HtdwcAnimatedTerminal.tsx` (accessibility-first), `DISTRIBUTION.md` (shot-list beats 4–5 + both social posts + checklist), `README.md` audit snippet.

### Pending
- **Not yet published.** This is the substantive content for **alpha.7** — bump `package.json` (still `1.0.0-alpha.6`) + the 3 hardcoded VERSION constants, rebuild, publish. Republish was already a prerequisite before recording the screencast (npm alpha.6 still serves the pre-May-29 pattern-citation output); this batch is what should ship in it.
- **Not committed.** Working-tree only, pending review.
- Positioning copy on the live site still leads with the broad "senior designer" line — the research suggests testing an accessibility+CI *hook* (senior designer whose first move is accessibility). Copy experiment, not a rebuild; deferred to a decision.

## May–June 2026 — post-alpha.6 work

Everything after alpha.5. Detail per session is in `docs/SESSION-LOG.md`; this is the roll-up. **The through-line of this stretch is a positioning correction:** dwic's live identity is **"a senior designer inside your terminal,"** with auditing as the lead capability — not the "design auditor" identity the older sections of this file assert.

### alpha.6 (2026-05-05) — pattern-citations, later reversed
- [x] Shipped `@imrandwc/dwic@1.0.0-alpha.6`: audit findings cited AI UX patterns from aiuxdesign.guide (`src/data/aiex-pattern-map.ts` + `aiex-patterns.ts`), plus a scrolling brand-logo strip on the landing under the "See demo" CTA. 22/22 suites green. Commit `3da1132`.
- ⚠️ **This feature was removed May 29** (see below). The alpha.6 pattern-citation work is superseded; the logo strip / ecosystem cross-promo to aiex was kept separately.

### `/how-to-design-with-claude` cornerstone page (2026-05-07)
- [x] Shipped a single-page SEO cornerstone at `/how-to-design-with-claude` — 9 narrative sections, animated terminal (`HtdwcAnimatedTerminal.tsx`), scroll-driven reveals (CSS `animation-timeline: view()`), copy-to-clipboard install pill, Article JSON-LD, sitemap priority 0.9. Targets the exact brand-name search query the domain already owns. Nav simplified to `dwc · How to design · Free library`. Commits `f52668c` + `cf19d25` (added to `llms.txt` + shipped IndexNow key).

### `/api/health` Supabase keep-alive (2026-05-12)
- [x] Added `web/app/api/health/route.ts` — cheap `count` probe on `profiles`, returns `200 {ok}` or `503` with a reason code. Lets an external cron (cron-job.org, every 3 days) keep the free-tier Supabase project from auto-pausing after 7 days idle. Load-bearing: if Supabase pauses, `/api/get-started/mint` 500s, `/api/events` rejects, `/admin/*` 503s, MCP onboarding/gating break. Commit `77d8c5d`.

### Positioning + honesty pass (2026-05-29)
- [x] **Positioning corrected.** Live identity is **"dwic — a senior designer inside your terminal"** (`web/app/layout.tsx` + `GetStartedPreview.tsx`), NOT "design auditor." Auditing is the lead capability, not the identity. Saved to memory as `project_dwc_positioning.md`. This file's alpha.4–alpha.5 "design auditor" framing is stale.
- [x] **Dropped the aiex pattern-citation feature (option A).** ~14 of 18 category→pattern pairings were non-sequiturs; the feature only existed to back-justify a "from aiuxdesign.guide" caption. Removed the "Related AI UX patterns" block from `src/audit/dashboard.ts` + `src/audit/markdown-report.ts`; deleted `src/data/aiex-pattern-map.ts` + `aiex-patterns.ts`. aiex cross-promo (`EcosystemLinks` + "Pattern library →" card) preserved separately.
- [x] **Honesty cascade.** Fixed now-false live claims exposed by the removal ("audits against AI UX patterns," "Findings cite AI UX patterns," the fake citation line in `HtdwcAnimatedTerminal.tsx`, meta description).
- [x] **New `/how-it-works` page** — 8 sections including "What dwic doesn't do" (boundaries) and "One brain, two surfaces" (the free `commands/*.md` library *is* dwic's knowledge base; 12 of 41 power the MCP specialists). Third nav link, sitemap 0.9.
- [x] **README** rewritten product-first (senior-designer headline; command count 44→41 verified; removed 4 deleted commands). Homepage demo numbers aligned to real audit output (`24 · 6 · 13 · 5`).
- [x] **SECURITY — closed a real leak.** The admin password (guards user-email PII, shared with aiex) had been committed into a CLAUDE.md changelog in the **public** repo. Redacted the tree; user rotated both dwc + aiex `ADMIN_PASSWORD` + added a distinct dwc `ADMIN_SESSION_SECRET`, set in Vercel + redeployed. History copy left (old value now dead). New rule: `feedback_no_secrets_in_claude_md.md`.
- [x] **`DISTRIBUTION.md`** launch kit added (60s screencast shot-list + Discord/Reddit/X posts, senior-designer framing).
- [x] Commits `8b05297` (main pass) + `86ff415` (migrate `gist.design` → `llms.gist`). Verified green: root build clean; audit test suites pass; web `tsc --noEmit` clean; `/how-it-works` renders 200.

### Docs cleanup (2026-06-05)
- [x] Moved the per-session diary out of `CLAUDE.md` into `docs/SESSION-LOG.md` so it stops loading into every session. `CLAUDE.md` is now the lean durable reference; `/save` appends to the log. Commit `69173da`.

### Pending after this stretch
- **Move `web/` (backend/admin/gating) to a private repo** — top priority next session; admin guards user-email PII behind one password in a public codebase.
- **Record the 60s screencast** — the single gate before any distribution post.
- `RESEND_API_KEY` not set in Vercel prod — `/api/get-started/mint` returns `emailQueued: false` (flow still works; no confirmation email sent).
- Confirm any npm publish token pasted in chat during alpha.6 was rotated (manual, at npmjs.com/settings/imrandwc/tokens).
